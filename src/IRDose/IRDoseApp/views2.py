...
class IRDoseDetailView(LoginRequiredMixin, DetailView):
    ...
    def get_context_data(self, *args, **kwargs):
        ...
        #radionuc_type = radionuclideChoice.get(pk=request.POST['radionuclideChoice'])
        #radionuc_type = request.POST.get('radionuclideChoice')
        #print(radionuc_type)
...
class IRDoseUpdateView(LoginRequiredMixin, UpdateView):
    model = CumActivity
    #exclude = ['cumAct', 'doseAbs', 'slug', 'category', 'owner', 'location', 'CT_Patient', 'CT_Organ', 'CT_Target_1', 'CT_Target_2', 'name', 'Organ']
    form_class = CumActivityCreateForm
    login_url = '/login/'
    ...
    #   instance = form.save(commit=False)
    #   instance.owner = self.request.user
    #   return super(IRDoseUpdateView, self).form_valid(form)
...
def cumActivity(t,A):
   ...
   #   A[i] = CumActivity.objects.filter(name=t)
    #print(A[i])
    return '%.2E' % Decimal((np.trapz(A,t,axis=0) + A[-1]*np.exp(-t[-1]*(np.log(2)/(6.6457*24))) / (np.log(2)/(6.6457*24)) )*math.pow(10,6)*3600)
