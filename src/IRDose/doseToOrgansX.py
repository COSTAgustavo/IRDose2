...
target_1 = sys.argv[2]
target_2 = sys.argv[3]
cumActivity = float(sys.argv[4])
#cumActivity = 8.68010783453e14  
...
outFile = open('dosimetrie/stdGATE/output/'+argName+'SourceDose_'+sys.argv[4]+'.txt','w+')
#outFile = open('dosimetrie/stdGATE/output/'+argName+'SourceDose.txt','w+')

print('Dose Source = ', round(doseArrayT[b_source].sum()/b_source.sum()*cumActivity,6), '+-', round(sigmaSource*cumActivity,6) , '(',round((sigmaSource*cumActivity)/(doseArrayT[b_source].sum()/b_source.sum()*cumActivity)*100,6),'%)', file=outFile)
print('Dose Source = ', round(doseArrayT[b_source].sum()/b_source.sum()*cumActivity,6), '+-', round(sigmaSource*cumActivity,6) , '(',round((sigmaSource*cumActivity)/(doseArrayT[b_source].sum()/b_source.sum()*cumActivity)*100,6),'%)')

if target_1=='True':
    print('Dose Target_1 = ', round(doseArrayT[b_target_1].sum()/b_target_1.sum()*cumActivity,6) ,'+-', round(sigmaTarget_1*cumActivity,6), '(', round(sigmaTarget_1/(doseArrayT[b_target_1].sum()/b_target_1.sum())*100,6), '%)', file=outFile)
    print('Dose Target_1 = ', round(doseArrayT[b_target_1].sum()/b_target_1.sum()*cumActivity,6) ,'+-', round(sigmaTarget_1*cumActivity,6), '(', round(sigmaTarget_1/(doseArrayT[b_target_1].sum()/b_target_1.sum())*100,6), '%)')
if target_2=='True':
    print('Dose target_2 = ', round(doseArrayT[b_target_2].sum()/b_target_2.sum()*cumActivity,6) ,'+-', round(sigmaTarget_2*cumActivity,6), '(', round(sigmaTarget_2/(doseArrayT[b_target_2].sum()/b_target_2.sum())*100,6), '%)', file=outFile)
    print('Dose target_2 = ', round(doseArrayT[b_target_2].sum()/b_target_2.sum()*cumActivity,6) ,'+-', round(sigmaTarget_2*cumActivity,6), '(', round(sigmaTarget_2/(doseArrayT[b_target_2].sum()/b_target_2.sum())*100,6), '%)')
